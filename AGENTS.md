...
| Scout | `gpt-oss:120b-cloud` via Ollama (Haiku via `claude -p` is an accepted stand-in) | `SCOUT-<task>.md`, untracked: candidate files + the spec lines that bind; dies when the TASK file is written | branches, keys, verdicts |
...
```
{ echo "You are a Scout. Read-only. Output ONLY: (1) candidate files a builder would touch, (2) the spec headings that bind the task. Under 120 words."; echo "TASK: <one sentence>"; echo FILES:; git ls-files; echo "SPEC HEADINGS:"; grep -n '^#' NIP-DRAFT*.md CONVENTION.md; } | ollama run gpt-oss:120b-cloud --nowordwrap > SCOUT-<task>.md
```
...
- 2026-09-11 Scout is gpt-oss:120b-cloud. Benched 7b, devstral:24b, qwen 32b and the cloud model on three tasks with known answers: the recipe was already right, the 7b was the misfire. The 32b and the cloud model tie; cloud is 11x faster, so cloud. Haiku is fine too.
