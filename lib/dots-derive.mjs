...
// One hop out: each source resolves to its whole subject — every notary carrying that `d`, duplicates included — and
// its accepts AND submissions arrive as grey `via` dots. `via` names the notaries the card sits under, `viaAcc` the ones
// that accepted it. A pointer, never adoption: an id the context already shows (own) is skipped; the viewer's own acts sit on top.
export function viaOf(context, notaries, projections, cards, own = new Set()){
  const n = notaries.get(context); if (!n) return [];
  const byId = new Map(), ds = new Set(sourcesOf(n).map(dOf));
  for (const c of notaries.keys()) if (ds.has(dOf(c))) for (const x of dotsOf(c, projections, cards)){
    if (own.has(x.id)) continue;
    const cur = byId.get(x.id) || byId.set(x.id, { ...x, kind:"via", via:[], viaAcc:[] }).get(x.id);
    cur.via.includes(c) || cur.via.push(c); if (x.kind === "acc") cur.viaAcc.push(c);
  }
