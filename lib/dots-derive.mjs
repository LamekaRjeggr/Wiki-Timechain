...
// A notary's sources: the 30829 coordinates on its `a` tags, itself excluded. The tether lives on the head, not in an act.
export const sourcesOf = n => [...new Set(n.tags.filter(t => t[0] === "a" && /^30829:[0-9a-f]{64}:.+/s.test(t[1])).map(t => t[1]))]
  .filter(c => c !== "30829:" + n.pubkey + ":" + (n.tags.find(t => t[0] === "d") || [])[1]);

// One hop out: what the context's sources accepted, as grey `via` dots naming the sources that hold each. A pointer,
// never adoption: an id the context already shows (own) is skipped; the viewer's own acts sit on top of the rest.
export function viaOf(context, notaries, projections, cards, own = new Set()){
  const n = notaries.get(context); if (!n) return [];
  const byId = new Map();
  for (const s of sourcesOf(n)) for (const x of dotsOf(s, projections, cards)){
    if (x.kind !== "acc" || own.has(x.id)) continue;
    (byId.get(x.id) || byId.set(x.id, { ...x, kind:"via", via:[] }).get(x.id)).via.push(s);
  }
