 ...
 */
function parseSelectors(selects) {   // one grammar for acceptance and refusal
  if (selects.length === 0 || selects.some((tag) => tag.length !== 2)) return fail("invalid-select-tags");
  ...
  const fields = wholeCard ? [...CORE_FIELDS] : selectorNames.map((selector) => SELECTOR_TO_FIELD.get(selector));
  return { ok: true, wholeCard, fields };
}
...
export function validateAct(event, { verifyEvent, strictTags = true } = {}) {
  ...
  const refuseTags = event.tags.filter((tag) => tag[0] === "e" && tag[3] === "refuse");
  ...
  if (allETags.length !== 1) return fail("invalid-e-tags");   // one e: its marker names the shape
  const isAcceptance = sourceTags.length === 1;
  const isRevoke = revokeTags.length === 1;
  const isRefusal = refuseTags.length === 1;
  if (!isAcceptance && !isRevoke && !isRefusal) return fail("hybrid-or-unknown-shape");
  ...
  const selection = parseSelectors(selects);
  if (!selection.ok) return selection;
  const { wholeCard, fields } = selection;

  if (isRefusal) {   // a refusal hides one exact version; it preserves nothing, so it carries nothing
    const target = refuseTags[0];
    if (target.length !== 4 || !HEX_64.test(target[1])) return fail("invalid-refuse-target");
    if (snapshots.length !== 0 || credits.length !== 0) return fail("refusal-has-acceptance-tags");
    return { ok: true, act: { ...common, type: "refusal", targetEventId: target[1], fields, wholeCard } };
  }
...
/**
 * Refusals are a set, not a sequence: a version is refused for a field while any
 * unrevoked refusal in that context names both. No register is touched.
 */
export function foldRefusals(refusals, { revokedTargetIds = new Set() } = {}) {
  const byTarget = new Map();
  for (const act of refusals) {
    if (act.type !== "refusal") throw new TypeError("foldRefusals accepts only normalized refusals");
    if (revokedTargetIds.has(act.id)) continue;
    const key = `${act.context}\u0000${act.targetEventId}`;
    const cur = byTarget.get(key) || byTarget.set(key, { context: act.context, eventId: act.targetEventId, fields: new Set(), actIds: [] }).get(key);
    act.fields.forEach((field) => cur.fields.add(field));
    cur.actIds.push(act.id);
  }
  return [...byTarget.values()]
    .map((r) => ({ ...r, fields: CORE_FIELDS.filter((f) => r.fields.has(f)), actIds: r.actIds.sort() }))
...
export function fold8828(events, options = {}) {
  ...
  const refusalActs = valid.filter((act) => act.type === "refusal");
  const targetById = new Map([...acceptances, ...refusalActs].map((act) => [act.id, act]));   // a revoke lifts either
  const validRevokes = [];
  const revokedTargetIds = new Set();

  for (const revoke of valid.filter((act) => act.type === "revoke")) {
    const target = targetById.get(revoke.targetActId);
    if (!target) {
  ...
  return {
    ...
    refusals: foldRefusals(refusalActs, { revokedTargetIds }),
    acceptedActs: [...acceptances, ...refusalActs, ...validRevokes].sort(compareActs),
    rejectedActs,
    ignoredEvents,
  };
}
